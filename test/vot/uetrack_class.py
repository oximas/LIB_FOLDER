...
class UETrack(object):
    def __init__(self, tracker_name='uetrack', para_name='uetrack_base'):
        # create tracker
...
def run_vot_exp(tracker_name, para_name, vis=False, out_conf=False, channel_type='color'):
    ...
    save_root = os.path.join('/media/kb/2A57A961A1CAB688/vis', para_name)
    if vis and (not os.path.exists(save_root)):
        os.mkdir(save_root)
    tracker = UETrack(tracker_name=tracker_name, para_name=para_name)
    ...
    if vis:
        ...
        save_v_dir = os.path.join(save_root,'001')
    ...
    while True:
        ...
        if vis:
            ...
            print(imagefile)
            image_name = imagefile[0].split('/')[-1]

            save_path = os.path.join(save_dir, image_name)
            image_b = image_ori.copy()
            image_b = cv2.UMat(image_b[:,:,3:])
